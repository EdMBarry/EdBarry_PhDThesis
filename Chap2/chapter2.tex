...
The following chapter is based upon work which has been published during the course of the PhD programme with a substantial number of extensions needed to support later chapters in this thesis.\\
...
\begin{table}[h]
	...
	\begin{tabular}{|c|l|l|}
		...
		\hline
		\textbf{Edward Barry}				& writing of text 					& 50\\
															& proof-reading							& 60 \\
															& theoretical derivations 	& \\
															& numerical calculations 		& 80\\
															& preparation of figures 		& 80 \\
															& initial concept						& 10 \\
		\hline
		Daniel Puyol							& writing of text 					& 20\\
															& proof-reading							& 10 \\
...
\noindent An ideal pH model has been implemented in the model in a similar fashion to the ADM1 model \cite{batstone2002}. The implementation includes phosphate acid-base pairs without ion pairing or activity correction. This is sufficient for domestic wastewater, but for streams with extreme pH, high levels of pollutants, or for implementation in a more generic benchmark simulation framework, this model should be extended to include non-ideal ionic activity and ion pairing as per the PCM-1 for wastewater treatment \cite{batstone2019}.  Temperature has not been included as a term in either the pH model or in the PAnM, but van't Hoff and Arrhenius expressions can be implemented for important parameters as previously investigated \cite{hulsen2016a}. \\
...
\subsubsection{Phototrophic continuous simulations}
...
\textit{Photoheterotrophic growth}\\
...
\item This model includes a lumped mixed culture phototrophic biomass. The inclusion of other heterotrophic biomass would be useful when a radiative field is included in the model as non-phototrophic organisms would attenuate the radiative field in a different manner.
...
A key priority for future research should be inclusion of poly-P and PHA accumulation as well as N\textsubscript{2} fixation and side H\textsubscript{2} production, as these processes (poly-P without carbon, PHA without oxidation or organics, and N\textsubscript{2}/H\textsubscript{2} production) are unique to photoanaerobic organisms. The topic of infrared light delivery has not been addressed in detail in this model, and is generally assumed to be in excess (i.e., not limiting catabolic rate, such that there is no mixed photo-dark fermentation (mixotrophic growth). This could be incorporated by limiting light to enable mixotrophic growth through the existing switch function that considers also spatial separation to dark zones, but as stated above, further work is required to consider accumulation and depletion of storage compounds. 
