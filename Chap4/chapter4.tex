...
In addition to organics and radiative field limitations, the third constraining factor is the separation of the solid phototrophic biomass from the liquid phase. As an added effect of segregating the solid phase during biomass growth, biofilms increase the light delivery to phototrophic bacteria, because planktonic biomass are much less abundant in the liquid phase, leading to less self-shading.
...
Case 1 has been treated as the base case for the simulations. The irradiance was initiated below the substratum, and the biofilm was initiated as a uniform layer on the substratum with values as per Table \ref{tab:biofilm_cases}. Figures \ref{fig:case1_ppb_frac}, \ref{fig:case1_growth_frac} and \ref{fig:case1_dist_frac} show the volume fraction of PPB, its apparent growth rate and the spatial distribution of particulates and water at four points in time: 1.0 d, 4.0 d, 7.0 d, and 10.0 d. 
\begin{figure}[H]
    \centering
...
\subsection{Specific phototrophic activity}
...
\subsection{Volume averaged irradiance and growth rate over time}
Figs. \ref{fig:vol_averaged}a and \ref{fig:vol_averaged}b show the volume averaged irradiance and the volume averaged growth rate over time, respectively.The volume averaged growth rate at 3 Wm\textsuperscript{-2} (case 2) was low due to the insufficient irradiance for phototrophic growth. There was still net biofilm growth (0.5 kgCOD m\textsuperscript{-3} d\textsuperscript{-1}) mainly due to chemoheterotrophic growth. For the cases where the radiant energy was deployed from below the substratum (cases 1 and 4), there was sufficient irradiance for phototrophic growth. The difference in these simulations was that case 1 began with a uniformly distributed biofilm whereas case 4 began with a sparsely initialised biofilm. After 4 days of simulated time, both the volume averaged irradiance and the volume averaged specific phototrophic activities for these two cases converged and followed very similar patterns for the remainder of the simulated time. This is due to the spreading of the biomass in a uniform manner within the biofilm. At the end of 10 simulated days, the growth rates are 2.45 kgCOD m\textsuperscript{-3} d\textsuperscript{-1} for case 1 and 2.8 kgCOD m\textsuperscript{-3} d\textsuperscript{-1} for case 4. 
\skippingparagraph
For the simulation conditions where the radiant energy was deployed from above the domain, there was a lag in specific growth rate for the sparsely initialised biofilm (case 5) compared with the uniformly initialised biofilm (case 3). This can be explained by the fact that the phototrophic biofilm was initially closer to the radiative field when compared with case 5. The biofilm in case 3 experienced 2 days of growth before reaching a steady state of growth of 2.5 kgCOD m\textsuperscript{-3} d\textsuperscript{-1}. The biofilm in case 5 experienced 8 days of growth before reacting a steady state. Both cases had similar volume averaged irradiance throughout the simulations, with the sharper initial rate in decline for case 3 (the uniformly irradiated biofilm) being due to the greater volume of biomass initially in the system. 
...
\subsection{Summary of controlling mechanisms}
The development of this model focused on the growth of phototrophic biofilm in the presence of a radiative field. Upstream processes such as the attachment of planktonic organisms to a surface, and downstream processes such as sloughing and detachment were not explicitly modelled in this work, but have been identified as requiring future work.
\skippingparagraph
With respect to growth processes, it was identified that delivery of substrate (hydrodynamics) and the delivery of radiant energy were the main potential limiting mechanisms. Accounting for the assumptions made around the void fraction, the volumetric growth characteristics of the biofilm, and the diffusivities of soluble substrates, it was found that the light delivery rather than the transfer of substrate was the main limiting mechanism. The density increasing mass of biofilm over time meant that the radiative field was attenuated at an increasing rate until a steady state was reached. In the case where a limiting irradiance was deployed at 3 Wm\textsuperscript{-2}, there was very little phototrophic activity, and chemoheterotrophic growth was effectively negated by the decay rate of the biomass. Where radiant energy was introduced from above the biofilm, a less uniform growth profile was evident, as the biofilm was attracted to the most intense point of the radiative field overhead. The model was effective in analysing the balance between substrate transfer, light attenuation, and biofilm growth, however detachment and attachment mechanisms were not included. 

\subsection{Limitations of the model}
\skippingparagraph
A phototrophic biofilm model was developed using computational fluid dynamics methods (CFD). Biomass growth and its coupling to a radiative field has been described, however there still exist some important aspects of the biofilm life-cycle which have not been developed in this work, but which merit discussion. The life-cycle of a biofilm includes the attachment of planktonic organisms to a substratum, growth and decay processes of the microbial community, the response to external hydrodynamic fields, and biofilm sloughing.
\skippingparagraph
...
\skippingparagraph
It was found that the radiative field was more limiting for biofilm growth than diffusive limitations of soluble substrates. There were five simulated cases exploring low intensity radiant energy, radiation from above the biofilm, radiation from below the biofilm, and sparse initial biofilm attachment. From these cases, there were two main classes which showed different behaviour: radiant energy from below the biofilm, and from above. For both the uniform and sparse cases with irradiance from below, the biofilm grew in a mostly uniform, tightly packed manner. As the radiative field adjacent to the biofilm, new growth occupied the irradiated space, pushing old biofilm away from the irradiated region. For the cases where the radiative field was initialised from above the domain, the biofilm morphology was slightly different. The biofilm was drawn to the regions where the radiative field was highest. Due to the scattering of the radiative field, it formed a parabolic front to which the biofilm was attracted. This resulted in a biofilm profile that was slightly stretched upwards and towards the centre of the domain where the irradiance was highest. This behaviour could not have been captured in a one-dimensional simulation, and therefore it is recommended that multidimensional simulations be carried out, especially for photobiofilms where the spatially varying radiant energy plays such an important role in biofilm growth. 
\skippingparagraph
Several limitations were identified, such as the fact that the biokinetic model included only phototrophic biomass which meant that the analysis of mixed culture biofilm morphology could not be explored. Additionally, this model was only concerned with the growth processes of an already attached biofilm. Attachment and detachment processes were not described in the model, and their inclusion would mean a more complete description of the phototrophic biofilm life-cycle.

%The biokinetic model was only defined in terms of a single lumped phototrophic biomass, and it is hypothesised that an upgrade to the biokinetic matrix would be necessary to appropriately describe the spatially varying biofilm community, especially because a radiative field could lead to quite different communities within a biofilm matrix. There was no externally induced flow field which could have affected the growth and the eventual sloughing of biofilm, and this was highlighted as a point to improve for future iterations of the model. Finally, as organisms can express different genes and phenotype depending on whether they exist in biofilm or in planktonic state, an extensive laboratory study would be required to re-calibrate and validate this CFD model for phototrophic biofilm purposes.